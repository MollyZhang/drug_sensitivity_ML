...
for (i=1; i<= 6; i++) {
    ...
    for( String gene : genes ){
        ...
        for (String drug : drugs ){
            ...
            m.add rule : ( ToPredict(C, drugArg) & DrugTarget(drugArg, geneArg) & Essential(C, geneArg) ) >> Sensitive(C, drugArg),  weight : 100, squared : false
        }
    }
    m.add rule : ~Sensitive(C, D),  weight : 100, squared: false
    ...
    def result_file = new File("result/essential_overlap/gene_drug_specific_rule_not_squared_both/fold${i}_result.txt");
    result_file.write ""
