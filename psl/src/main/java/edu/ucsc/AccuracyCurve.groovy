...
weight_list = file.text.split("\n")[1..-1]

weight_list.eachWithIndex { weights, index ->
    println " " 
    println "Iteration ${index}:"
    // parse weight for this iteration
    active_weight = Double.parseDouble(weights.split("\t")[0]);
    essen_weight = Double.parseDouble(weights.split("\t")[1]);
    prior_weight = Double.parseDouble(weights.split("\t")[2]);
    ...
    def result_file = new File("result/log/weight_curve/${index}.txt");
    result_file.write ""
